...
//   - 层内：前提权重求和（premiseRegistry.getWeightSum——前提返回值即权重，erArk `now_weight += premise_judge`）
...
function buildPremiseCtx(charId: string): Record<string, any> {
  return { sourceId: charId, selectedCharacterId: charId }
}

// 注释：单前提求值（erArk search_target 语义：任一前提 <= 0 淘汰，求和即权重；
// strict=true：未知前提 → 0（淘汰）——AI 目标前提缺失是数据错误，显式暴露而非静默放行。
...
function evalPremiseSum(premise: string, charId: string, premiseCache: Map<string, number>): number {
  ...
  if (!premiseRegistry.getRegisteredIds().includes(key)) {
    reportOnce(
  ...
  let weight: number
  try {
    weight = premiseRegistry.getWeightSum([premise], buildPremiseCtx(charId), true)
  } catch (e) {
