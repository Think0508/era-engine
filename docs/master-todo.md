...
### 属性面板完整性检查（后续对接 mod 扩展时逐项确认）

- [ ] 感觉（皮肤感度等）——有定义/有展示，但无代码修改它们
- [ ] 能力（技巧/顺从等）——settle 公式会改，ability-progression 管升级，但真实游戏流程中是否触发需验证
- [ ] 刻印——h-mark 插件注册了，但需要完整的 H 场景来触发
- [ ] 技能（华山剑法等）——ability-progression 管升级，但升级途径（gain_ability_xp）需要指令/事件来调用
- [ ] 技术（指技等）——有定义但无人修改
- [ ] 角色面板所有分组在真实游戏流程中是否有数据可看
