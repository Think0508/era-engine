...
> 相位决定**什么时候**生效；**改哪个数值**见下一节「伤害数值链（中间值 → 落点匹配手册）」——
> 同一个统计键在不同相位会落到数值链的不同位置（如 `damage_out` 有②/⑤两个落点）。
...
| `防御` | 数值链⑧（扣防前那一项） | flat = 平加、percent = 倍率、`set 0` = 无视防御 |
| `最终伤害` | 数值链⑨→⑩（**扣防御之后**） | flat = 平加、percent = 倍率、`set 0` = 该次伤害归 0。**攻守双方写入合并后只应用一次**（求解见下节） |
...
**多来源叠加规则**（同一通道被多个效果/天赋写入时）：

| 分量 | 合并方式 | 后果 |
|---|---|---|
| `flat` | **相加** → 加到基准上 | 平加在 percent **之前**，所以平加值会被 percent 放大 |
| `percent` | **相加** → 最后只做**一次**乘法 | `+10%` 与 `+20%` → `×1.30`（**不是** 1.1×1.2）。所谓"×2"必须写成 `percent = 1.0`，与别的 percent 相加而非相乘 |
| `set` | **后者覆盖**（合并顺序：常驻包 → 各相位 overlay → 天赋包） | `set` **替换基准**（原值被丢弃），但 `flat/percent` 仍作用其上 |

例：基准 100，`flat +100` + `percent +50%` → `(100+100)×1.5 = 300`（**不是** `100×1.5+100 = 250`）。
...
**这一节解决一个问题**：我要写"伤害 ×1.5 / +10% / 减半"这类效果时，该挂哪个中间值？
下面先给**命名中间值的有序定义**（名字 = 公式日志 `parts` 里的键名，`@公式明细` 指令可实时查看），
再给**"我想改 xx → 用哪个落点"对照表**。

### 命名中间值（每段命中各算一次，按发生顺序）

```
① 公式输出      = base_damage 钩子返回的 value（唯一权威定义见「武侠公式」节）
                  ├ 力道项/灵敏项 → 武功威力 → 三项小计 × 武功系数倍率 × 风格系数 × 精通系数
                  │  + 内力项 + 其他加成
                  ├ × (1 + 天赋伤害加成)        ← combat_damage 类天赋在这一步
                  └ + 即时毒伤 M′               ← 毒在这一步并入（所以即时毒伤吃暴击/浮动/防御）
② 基础伤害      = max(0, 公式输出 × (1 + damage_out))          ← 日志键「基础伤害」
③ 暴击率/暴击倍率（damage_crit 相位求值；不暴击则倍率 = 1）
④ 输出伤害      = max(0, round(基础伤害 × 暴击倍率 × 浮动系数))  ← 日志键「输出伤害」
⑤ （damage_output 相位的 damage_out 在④之后再乘一次，**不**计入「输出伤害」）
⑥ （damage_on_target 相位的 cancel → 此后归 0）
⑦ 扣防前        = max(0, round(输出伤害 × (1 − 减伤)))          ← 日志键「扣防前」
                  减伤 = damage_in（常驻 + damage_on_target/damage_mitigate 相位）
⑧ 防御          = (根骨×0.8 + 定力×0.5) × (1 + defense_mult) → 通道「防御」
                                                                ← 日志键「防御」
⑨ 扣防御后      = max(0, 扣防前 − 防御)                        ← 日志键「扣防御后」
⑩ 最终伤害      = max(0, round(扣防御后 过通道「最终伤害」))     ← 日志键「最终伤害」
⑪ 实际扣血      = min(最终伤害, 目标当前 HP)                    ← 溢出部分丢弃（过量伤害不计）
```
...
| 陷阱 | 说明 |
|---|---|
| **「最终伤害」≠ 实际扣血** | ⑩ 是公式结果；⑪ 才是真扣掉的血。目标血少时⑪ < ⑩，溢出不记账。饮血（`leech_hp_from_damage`）读的是⑪ |
| **「基础伤害」= 过了 damage_out 之后的值** | 同名键在 `base_damage` 记录里被覆写两次（公式自身 → +天赋 → +damage_out），**最终显示的是②**。要看公式本体用①的其他分项（三项小计/风格系数…）反推 |
| **`damage_out` 有两个落点** | **常驻**（无 trigger）与 `attack_pre`/`attack_launch`/`hit_roll`/`on_hit`/`damage_base` 相位 → 落在②；**`damage_output` 相位** → 落在⑤。两者都是乘法，净结果相同，但**日志里体现的键不同**（前者改「基础伤害」，后者只改「扣防前」） |

### 我想改 xx → 用哪个落点

| 你想改的东西 | 落点 | 写法 | 生效位置 |
|---|---|---|---|
| 公式里的力道分项（力道×3） | 通道 `力道项` | `percent`（`-0.3` = 减三成） | ① 之前 |
| 公式里的灵敏分项（暗毒系） | 通道 `灵敏项` | `percent` | ① 之前 |
| 该段武功威力 | 通道 `武功威力` | `flat` / `percent` / `set` | ① 之前 |
| 风格系数（×1.1 类） | 通道 `风格系数` | `flat` / `percent` / `set` | ① 之前 |
| 公式末尾平加伤害 | 通道 `其他加成` | `flat` / `set`（**percent 对 0 基准无效**） | ① 末尾 |
| **「基础伤害」×1.5 / +10%** | 统计键 `damage_out` | `percent`（`0.5` = +50%） | ②（`damage_output` 相位则 ⑤） |
| 命中率 +10 点 | 统计键 `hit_bonus` | `flat`（**点数**） | 命中判定 |
| 对方闪避 −10 点 | 统计键 `dodge_bonus` | `flat`（点数） | 命中判定 |
| 闪避/命中能力减半 | 通道 `准头` | `percent -0.5` | 命中判定 |
| 必中 / 命中率覆盖 | 通道 `命中率` | `set 999`（**percent 对 0 基准无效**） | 命中判定 |
| 稳定输出（关掉浮动） | 通道 `浮动系数` | `set 1.0` | ④ |
| 暴击率 +10 点 | 统计键 `crit_rate` | `flat`（**点数**） | ③（常驻值归钩子读，见下「归属契约」） |
| 暴击倍率 +0.5 | 统计键 `crit_mul` | `percent` | ③（同上） |
| **受到的伤害 +50%（易伤）** | 统计键 `damage_in` | `percent` **负值**（`-0.5`） | ⑦ |
| 减伤 30% | 统计键 `damage_in` | `percent` 正值（`0.3`） | ⑦ |
| 防御减半（破甲） | 统计键 `defense_mult` | `percent -0.5` | ⑧ |
| 无视防御 | 通道 `防御` | `set 0` | ⑧ |
| **最终伤害 ×0.5 / +N 点** | 通道 `最终伤害` | `percent` / `flat`（`set 0` = 该次伤害归 0） | ⑩ |
| 毒伤减免（即时+持续） | 通道 `毒伤害` | `flat` / `percent` / `set 0` | 毒伤害 |

> 三个易混落点的区别：**降低伤害**有三条路，作用位置不同——
> ① `damage_in` 正值（比例，⑦ = 扣防御**之前**，对高防御目标收益更大）；
> ② 通道 `防御`（改⑧那一项，平加/倍率/`set 0` 无视防御）；
> ③ 通道 `最终伤害`（⑩ = 扣防御**之后**的最后一道，`flat` 就是"最终伤害 ±N 点"的字面语义）。
> 想"目标血少时也至少打掉 N"用①；想"精确 ±N 点"用③。

### 常量 vs 可改的边界

不是所有中间量都开了接口。判据 = ①天赋常改 ②改属性达不到 ③改完影响本次计算；
被**刻意剪掉**的（改属性即可表达、或已有统计键落点）：`武器项`/`武功系数`/`精通系数`/`内力项`/
`基础命中率`/`K`/`准头比率`、以及 `基础伤害`/`受伤`/`暴击率`/`暴击倍率`（= 既有统计键）。
**要加回来 = 通道注册表加一行 + 公式里读一行**（见 `combat-wuxia/formula.ts` 的 `WUXIA_CHANNEL_DEFS`）。

### 天赋 modifiers 的单位（⚠️ 各 formula 不一致）

`docs/talent-system.md` 把 `plus` 写作"加法值"、`multiply` 写作"乘法系数"，但**实际换算按 formula 分档**
（`combat-wuxia/index.ts` 的 `TALENT_FORMULA_STATS` + 编译分支）：

| formula | `plus` 的含义 | `multiply` 的含义 |
|---|---|---|
| `combat_channel` | `flat`（平加原值） | `percent`（比例） |
| `combat_damage` | **百分数**（`plus = 50` → `+50%`，内部 `plus/100`） | 比例（`0.5` → `+50%`） |
| `combat_hit` / `combat_dodge` / `combat_crit`（点数组） | **点数**（`plus = 10` → +10 点） | 比例 × 100（`0.1` → +10 点） |
| `combat_crit_mul` / `combat_defense` / `combat_in`（倍率组） | **比例**（`plus = 0.5` → +50%） | 比例（`0.5` → +50%） |

同一句 `plus = 50` 在 `combat_channel` 是"+50 原值"、在 `combat_damage` 是"+50%"、在 `combat_crit` 是
"+50 点暴击率"——**写天赋时先查这张表，别照抄别的 formula 的数字**。
另：`combat_damage` 走独立的 `talentDamageMods`（在 ① 内以 `×(1+Σ)` 求值），**不会**再建常驻
`damage_out` 条目（避免双计费）；其余 `combat_*` 编译为**常驻 `modify_stat` 条目**进效果区。

> **统计键的归属契约**（防双重计费，2026 修订）：分两组，写钩子/加效果前先确认在哪一组——
> **A 组**（`hit_bonus` / `dodge_bonus` / `crit_rate` / `crit_mul` / `defense_mult`）：**常驻值由公式钩子
> 自己读** `ctx.source.stats` / `ctx.target.stats`，combat-base 只把**相位 overlay** 叠到钩子返回值上。
> **B 组**（`damage_out` / `damage_in`）：常驻值由 **combat-base 自己**在其数值链位置应用，钩子不读。
> 曾经 `crit_rate`/`crit_mul` 两组都读 → 常驻 `+50` 点实测变 `+100` 点、`crit_mul +1.0` 变 `×3.5`；
> 现已修正为 A 组，并有回归测试（`combat-wuxia.test.ts` 的「统计键计费」describe）。
...
float_mul / defense_value / final_damage / is_attack_skill`
链式：`battle_start / combatant_init（被动/天赋编译）/ turn_start（动态技能指令挂载）/ turn_end`

- `final_damage`：数值链⑨→⑩之间调用，ctx 额外带 `input` = 扣防御后的值；返回最终伤害
  （不注册该钩子 → 原值透传）。这是"最终伤害 ×0.5 / ±N 点"的落点，由 combat-wuxia 实现为通道「最终伤害」。
