...
const STRUCTURAL_WHITELIST = new Set([
  ...
  '吐纳', '玄功', '玄门心法',
  // manual-system 测试夹具（manual.test.fixture.ts 的 manual-test 假 mod 专用——
  // 秘籍/能力/天赋/物品/品级/属性全部内联在该文件的 TOML 文本里，不进任何真实 mod 定义）
  '九阴真经', '初级拳法', '九阴真经心法', '九阴白骨爪', '摧坚神爪', '凌波微步', '基础掌法',
  '九阴天赋', '九阴人皮', '九阴真经上卷', '九阴真经·完整', '九阴真经总纲', '初级拳法秘籍',
  '龟息功', '第三内功', '野球拳', '三流', '绝世', '武学常识', '气血上限',
  // manual-system 负向测试：断言"结构路径合法但引用不存在的秘籍 ID"（故意未定义）
  '不存在',
  // combat-base 蓝耗提供者测试（2026-09-23）：运行时注入的假技能定义（不在任何 toml 里）
  '无耗写招式', '显式耗招式',
  // manual-system 加载期校验测试（manual-validate.test.ts）的内联夹具名——
  // 全是"故意写坏"的假定义/假属性（坏曲线/坏几何/坏内功…），不进任何真实 mod
  '测试技能', '测试天赋', '测试秘籍', '测试内功', '不存在属性',
  '坏卷册', '零上限', '超上限', '坏曲线', '坏几何', '线性', '指数', '数组', '几何',
  '坏内功', '好内功', '也',
])
...
const STRUCTURAL_NS = new Set([
  ...
  // 秘籍-技能系统（2026-09-23）——char.manuals['秘籍ID'] 是修炼进度查询、
  // char.kill_ledger['敌人键'] 是首杀计数（键都是 mod 自定义标识，非属性名）
  'manuals', 'kill_ledger',
])
