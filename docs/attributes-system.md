...
- ⚠️ **本层残留未决（2026-09-23 记录，均需人类裁定，本波刻意未实现）**：
  1. **跨属性家族里的反馈回路**：`src/plugins/h-core/effects/settle-effects.ts:371`（有效**阴茎大小** →
     基础**射精欲**增量，每次行动重复）与 `src/plugins/h-mark/index.ts:182-190`（有效状态 → 永久**刻印等级**，
     而刻印等级正是该状态的累加系数 = 自我放大）。纯系数型（`settle/favorability.ts:19` 状态等级→好感倍率、
     `settle/pain-adjust.ts` 润滑/阴茎大小→苦痛）**无回路、安全**，不需要处理。
     （注：「跨属性一次性换算」是否一律改走上面的换算规则，属**产品问题**，尚未裁定。）
  - ✅ **已关闭（2026-09-23 末轮）**：
