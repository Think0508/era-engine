...
- **⚠️ 写路径安全 ≠ 读-改-写安全（两种形态，2026-09-23 已全部清扫）**：上面三条路只保证"失效"正确，**不保证"读-改-写"正确**。
  ...
  **末轮补完**：`settleEjaDecay`（本文件最后一个"没加上限判据"的写回点，见下面的已关闭项）+
  换算规则（转珠）+ 为审计修过但没有"带修正"测试的站点各补一条最小回归
  （`realtime-settle.test.ts`「审计站点」组、`sleep-system.test.ts`、`h-core/anger-system.test.ts`、
  `h-core/index.test.ts`、`confinement-system.test.ts`、`binding-resolver.test.ts` 的 `getRawForPlugin` 组）。
- **规则（写新代码必守）**：**要写回哪个值，就从哪个值出发读**；白字（基础）会被写，绿字（加成）只用于显示与判断。
  ...
  - **上限/门槛判据仍用有效值**（`getEntityAttr` / `clampAttrValue`）——「上限 +500」这类修正的意义就是抬高上限，
    判超限时必须看到加成后的上限。`applyAttrDelta` 的 `clamp: true` 现在只钳**常量**上限（疲劳 160/信赖 300/
    好感 100000/饥饿 240/尿意 300/欲望 100/默认 99999）并保留「非负增量不得被钳制反噬」守卫
  - ✅ **属性上限改为「读时封顶」（2026-09-23 末轮用户裁定）**：`体力/气力/射精欲/精液量` 四项的上限由
    **有效值层**统一收口（`src/core/attribute-eval.ts` 的 `applyCapProjection`，与 compute/修正叠加同处）——
    **裸值可以越过上限，读出来的有效值不可以**：
    ...
    写入端因此**不再按属性上限钳制**（那是「临时上限修正沉淀进基础值」的通道：上限减益下
    `min(有效上限, 裸值+增量)` 会把裸值截断 —— base 体力 100 / 上限 120 挂 `体力上限 −100` 时
    `recover_permil +50` 曾把基础体力写成 **20**，撤修正仍 20 = 永久 −80；上限增益下又能让裸值爬过基础上限）。
    规则一句话：**上限是"读的约束"，不是"写的约束"**；判据（`clampAttrValue`）仍看有效上限。
    边界：投影只对 `ATTR_CAPS` 里带 `maxAttr` 的属性生效、上限必须 > 0（与 `clampAttrValue` 同约定）、
    上限属性不吃自己的规则；属性未被 mod 定义时不投影（沿用"无定义 = 恒等"闸门）。
    两条配套事实（**写新代码前必读**）：
    · **裸值越过基础上限后会怎样**：读不出来，但**留在存档里**。体力/气力有回收口 —— `clampHpMp`
      （`src/core/realtime-settle.ts`，`realtimeSettle` 入口末尾的收口一步）两端都读**裸值**，
      把 `裸值 > 基础上限` 的部分拉回基础上限；**射精欲/精液量没有回收口**（结算里没有对应钳位），
      越顶的裸值会一直留着（只是读不出来），直到该属性自己被扣减。
    · **`applyAttrDelta` 不再按属性上限（`maxAttr`）钳制**：`opts.clamp` 只钳**常量上限**（疲劳/信赖/好感/
      饥饿/尿意/欲望/默认 99999）与下限 0，并保留「非负增量绝不减少裸值」守卫（常量上限同样不得反向回收既有裸值）。
- 🧾 **换算规则：判定读有效值、换算读裸值（2026-09-23 末轮用户裁定，作者必守）**——
  当一个结算把**值换成永久物**（转珠、兑换、永久记账），两个操作数**分属两个域**，别写混：

  | 环节 | 读哪个域 | 为什么 |
  |---|---|---|
  | **存在性/阈值判定**（"够不够格换算"） | **有效值**（`getEntityAttr` / `bindingResolver.get`） | 临时 buff 就该帮忙达标；临时 debuff 把有效值压到 0 时**正当拦住**换算（用户裁定：这是 debuff 该有的威慑） |
  | **换算量 / 扣除量 / 等级衰减率** | **裸值**（`readRawAttr` / `bindingResolver.getRaw*`） | 被换算的值随后从**基础值**里扣掉 —— 两操作数同域才能"给的 = 扣的" |

  为什么不能整段读有效值：临时加成会**凭空铸永久货币**。已修实例（`src/core/juel-settle.ts` 每晚转珠）：
  `base 快乐 0 + 临时「快乐 +500」` → 判定成立（有效值 500）、若换算也读有效值就凭空铸珠
  （按 500 铸；erArk 等级衰减率下 Lv2 = 90% → 450 珠），可重复、免费、与玩家行为无关。
  改法：判定仍读有效值（debuff 归零照样拦住），
  换算量与清零量读裸值（`base 0 → addJuel 0`，而 `base 快乐 100 → 100 珠 + 清零` 逐位不变）。
  回归：`sleep-system.test.ts` 的「转珠换算读基础值」+「转珠换算正对照」两条。
  ⚠️ 被 debuff 拦住的那晚**基础值不会被清零**（值留在裸值域），debuff 撤掉后照常换算 —— 这是"拦住"而非"吞掉"。
- ✅ **跨属性家族里的反馈回路（2026-09-23 用户裁定，已按上面的换算规则收口）**：
  原残留 1 的两处「临时值 → 永久资产」转换都已落地**判定读有效值、换算读裸值**：
  1. `src/plugins/h-mark/index.ts`（`checkOne` → `thresholdMet`）——**刻印等级**换算读裸值，
     而它正是该状态此后的累积系数（`h-config state_ability`：快乐→快乐刻印/屈服→屈服刻印/苦痛→苦痛刻印…）
     → 读有效值等于「拿临时值铸永久等级，永久等级再放大该状态累积」＝自我放大。
     改法：**门槛判定**（够不够格升级）仍读有效值（临时 buff 帮忙达标 / 临时 debuff 正当拦住），
     **等级换算**读 `readRawAttr` ——`base 苦痛 0 + 临时「苦痛 +50000」`→ 不再铸出 LV3；
     `base 苦痛 50000`（无修正）→ 逐位不变仍 LV3。
     回归：`src/plugins/h-mark.test.ts` 的「等级换算读裸值」+「正对照」+「门槛判定仍读有效值」三条。
  2. `src/plugins/h-core/effects/settle-effects.ts`（`pl_p_adjust`）——`ownPFeel`
     （发起者当前 **P 快感 `ATTR.PENIS`（阴茎）**；erArk `status_data[3]`，早期注释误写作"阴茎大小"）
     读裸值后再折算进**基础**射精欲（`h-ejaculation` 的 `addEja` 写 `base[射精欲]`，每次行动重复）。
     `base 阴茎 240 + 临时「阴茎 +240」`：增量仍是 `floor(55 + 240/8) = 85`（修复前按有效值 480/8 → 115）。
     回归：`src/core/settle-fidelity.test.ts`「射精欲增量按**裸值**阴茎取」。
  纯系数型（`settle/favorability.ts:19` 状态等级→好感倍率、`settle/pain-adjust.ts` 润滑/阴茎大小→苦痛）
  **无回路、安全**，未改。
  - ✅ **已关闭（2026-09-23 末轮）**：
    · 原残留 1「`realtime-settle` 五处按有效上限写回」—— 唯一的裸值截断通道 `settleEjaDecay`（射精欲消退是
      **减量**路径，没有"裸值未达上限"的前置判据，临时「射精欲上限 −N」会把裸值截断）已改为**无上限钳制**
      （只保留下限 0；上限交给读时封顶）。`settleTired` / `settleHunger` / `settleUrine` / `settleSemen`
      继续调 `clampAttrValue`：它们都有"裸值未达上限则跳过"的前置判据，且其中的常量 `cap` 类
      （疲劳/饥饿/尿意）本就不参与读时投影 —— 写入端钳的只是**本次增量的幅度**，不会回收既有裸值。
    · 原残留 2「宝珠转换判定读有效值」—— 已按上面的**换算规则**落地（判定读有效值、换算与清零读裸值）。
...
- **属性修正**：由效果/装配/装备/状态挂上来的临时或条件性增减（`flat` / `percent` / `set`）。
  ...
  **修正来源共三类**：① 声明式（数据里写的 `attribute_mods`）② **运行时清单**（`char.attr_mods`，状态/战斗
  ...
  - ✅ **内功装配（2026-09-23 已接线）**：`char.equipped_abilities`（能力 ID 数组）+ 能力定义的
    `equipped_mods = [ { attr, flat?, percent?, set?, per_level? } ]`——**声明式来源第 4 条**，
    与装备/被动技能/天赋同代数（`per_level` 按该能力当前层数缩放）。装卸即生效/回落，无恢复代码。
    槽位规则（数量、"无限"哨兵值、准入校验）属玩法，在 `manual-system`（见 `docs/manual-system.md`）。
    装配/卸下入口：`ctx.api.call('internal','equip'|'unequip', charId, abilityId)`。
  - **今天仍未接线**：**跨实体修正**（甲的状态改乙的属性 —— 本轮修正一律作用于**目标自身**，
    招式打别人时"目标"就是被打的人）、条件手册标注受修正字段、UI 的基础/有效值差异呈现（`120 (+15)`）。
...
> **属性名的归属（2026-09-23 用户裁定）**：`ATTR` / `ATTR_CAPS` 定义在 **core**（`src/core/entity-utils.ts`）
> 并不违反「core 不认识属性名」的铁律——该铁律的准确含义是 **core 不认「不通用的」属性名**：
> mod/世界观专属的属性名（气血/内力/调教度…）不得进 core；而 **体力/气力/精力/精液量/射精欲** 这类
> 足够底层通用的原生属性名**合法地留在 core**，且**只此一处登记**（ADR-0013）。
> `ATTR_CAPS` 保持现状 —— core 单表维护（不走 mod 声明）。
