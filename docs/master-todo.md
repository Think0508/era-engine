...
- **战斗效果条目 v4.0（2026-09-12）**：一个效果 = **库定义 + 技能引用**（`abilities[].battle_effects = [{ effect = "名", 参数… }]`，只覆盖参数白名单；结构字段由库条目定死）。改了什么：① `delivery`(zone/instant)/`settle`/`apply_at` 三相位字段 + **施加器**注册表（`mount`/`apply_poison`/`apply_element`）② 数值三形态 `value = 数字 | {flat,percent} | {set}` + 每层乘性 `growth` ③ 追加 3 通道 `准头`/`力道项`/`灵敏项`（失势/致盲/截脉的落点）④ 毒合并为单条 + 层数模型（`stacks` 1/2/3 = 毒/猛毒/剧毒，`level_names` 管显示名）⑤ 火毒/寒毒（冰火相消：挂本元素 + 清自身 ≤x 层对立毒 + 引爆对方对立毒一次并清除）⑥ 新动作 `heal_hp`/`heal_mp`/`leech_hp_from_damage`(饮血=按实扣血量)/`extra_attack`(追击=同招再打一次、不递归、扣内力、`max_per_action` 限次) ⑦ `action_block` 语义改"该次行动作废、轮到下一位"（玩家与 NPC 一致，封穴=丢回合）⑧ `applyDamageTo` 返回实扣血量 + `attack_end` 相位带 `pendingDamage`；`damage_in` 去掉 ±0.9 截断（易伤无上限）。**删除**（不留兼容）：`apply_status`/`apply_effect` 战斗动作、`mode`、`k`、`merge_group`、`abilities[].effects`、隐式 `value×层数` 缩放、`accumulateStat` 的单位换算。数据：默认库 22 条（连绵/饮血/增伤/暴击两条/乘势/消内/吸内/追击/即时回血/即时回气/回血/回气/流血/破甲/失势/缓慢/致盲/封穴/截脉/破绽/毒/火毒/寒毒）已进 `docs/native-entries-catalog.md` 的「战斗效果」分类；`combat.getEffectCatalog`/`getParamVocab` 为将来的拖拽式技能编辑器备好数据。测试：+19 例（battle-effects.test.ts 真读默认库做契约校验 + 每条机制的实战断言）
- **战斗效果 / 战斗状态 术语与来源分离（2026-09-12 补充）**：明确「战斗效果条目」（技能词条，`delivery=instant`，每个相位现造现弃、不进效果区）与「战斗状态」（效果区里的 BUFF/DEBUFF，`delivery=zone`，id = 库条目名、可合并）**是两个对象**，术语不再混用"效果"；战斗状态实例新增 **`origin`（skill/passive/talent/system）+ `originId`** 并随 `getCombatState().effects[]` 暴露 —— UI 可把"被技能挂的 BUFF/DEBUFF"与"被动/天赋常驻条目"分组显示，不必解析 id 字符串。**回血/回气** 拆成即时版（`即时回血`/`即时回气`，instant@on_use，就地回复、不进效果区）+ 持续版（`回血`/`回气`，zone+self，settle=turn_end）共 4 条；跨来源同名（被动常驻 + 技能临时）**按来源各自独立**——已用测试钉死行为（不合并、不崩、两层叠加）
- 战斗外精确分钟级 tick
- NPC 队友 AI 优化
