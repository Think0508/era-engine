...
测试按"是否需要装载 mod/引擎数据"分两层（判据 = 文件内是否出现 `loadMod(`/`parseModData(`/`talkCommonOnEnable`）：

| 命令 | 内容 | 实测 |
|------|------|------|
| `npm run test:unit` | 54 个单元文件（core 机制/纯函数/插件隔离用例），**日常内循环用这个** | ~17s / 517 用例 |
| `npm run test:integration` | 77 个重集成文件（真实引擎+mod 数据装载） | ~93s / 1061 用例 |
| `npm run test` | 全量（= 两层之和） | ~120s / 1573 用例 |
| `npm run test:tiers` | 只打印分层清单与数量（排查用） | 即时 |
| `npm run validate` | 数据文件独立校验（脱离游戏） | ~数秒 |

> 分层不是"跳过测试"：`test:unit` + `test:integration` = 全量。分层前全量约 506s，
> 其中测试逻辑本身仅 46s——瓶颈是重集成文件各自冷启动装载 44MB 口上数据；
> 2026-09-11 惰性加载改造后全量约 120s（详见 `docs/talk-common-system.md` 惰性加载模型）。
