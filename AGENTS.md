...
> **澄清（2026-09-23 用户裁定）**：铁律里「不认属性名」的准确含义是 **core 不认「不通用的」属性名** ——
> mod/世界观专属的属性名（气血/内力/调教度…）不得进 core；而 **体力/气力/精力/精液量/射精欲** 这类
> 足够底层通用的原生属性名**合法地留在 core**，其唯一登记处是 `src/core/entity-utils.ts` 的
> `ATTR` 常量与 `ATTR_CAPS` 上限表（ADR-0013）。`ATTR_CAPS` 保持现状（core 单表维护，不改由 mod 声明）。
...
**核心原则**：引擎不认识任何**mod/世界观专属**属性名——它们都是模组自定义的字符串key。
（通用底层属性名如 体力/精力/精液量/射精欲 是例外：core 的 `ATTR` 常量 + `ATTR_CAPS` 单表登记，
见「最高架构铁律」后的澄清。）
...
## 架构合规铁律（不可违反）
- **三层绝对分离**：`src/core/` 不得出现任何具体玩法名词（**mod/世界观专属**属性名、模式名、系统名）——通用底层属性名（体力/气力/精力/精液量/射精欲…）是例外：它们以 `ATTR`/`ATTR_CAPS`（`src/core/entity-utils.ts`）为唯一来源合法留在 core（见「最高架构铁律」后的澄清）
