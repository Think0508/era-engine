...
**Parameter**:
A set of temporary physical and emotional numeric values that reset daily by default (similar to eraTW's Palam system), used for per-interaction calculations such as pleasure, obedience, arousal, and shame. Parameters are declared in `definitions/attributes.toml` with `daily_reset = true` and are displayed in the Parameter panel grouped by `display_group`. The standard set includes 10 pleasure sensations (皮肤/胸部/阴蒂/阴茎/阴道/后穴/尿道/子宫/口喉/心理) and 13 behavioral params (润滑/习得/恭顺/好意/欲情/快乐/先导/屈服/羞耻/苦痛/恐怖/抑郁/反感). Pleasure params use `level_thresholds` for 10-level grading. Parameters are mod-defined — adding a new parameter just means adding it to attributes.toml with `daily_reset = true`. The ParameterSection reads dynamically from attribute definitions at runtime. 尿道 is engine-supported but hidden by default (display=false) — mods opt in by defining its sensitivity abilities (ADR-0004).
_Avoid_: Hardcoding parameter keys in ParameterSection, treating parameters as permanent attributes, using erArk-specific terms like 兽部

**Sex position**:
A sexual intercourse posture, identified by numeric ID 1-12 (with -1 = no position). The position's pleasure coefficient is defined in h-config `[sex_positions]`; when a participant is in a position, V/A/U/W pleasure gains that coefficient (chara_feel_state_adjust:314-325). A character's favorite position (体位喜好 talent with `favorite_position` field, or derived from position experience 141-152 ≥ 100) adds +0.5; womb sex (子宫奸) adds +2 to W pleasure. Position is stored on the settled character's `h_state.current_sex_position`.
_Avoid_: Hardcoding position coefficients in engine code, treating position as a global scene state

**Ejaculation desire**:
The accumulated urge to ejaculate (射精欲), built up during H scenes. Every time P-part pleasure is settled, the second-stage settlement adds `100 + int(ejaculation_desire × 0.4)` (erArk ADD_SMALL_P_FEEL — per-character generalization of the fork's player-only behavior, ADR-0006). Additional sources: PL_P effects (being serviced) and explicit eja effects. When desire reaches its cap, ejaculation triggers (eja_climax). The field is owned by the h-ejaculation plugin — other plugins read/write it only through its public API.
_Avoid_: Writing 射精欲 directly from other plugins, player-only accumulation hardcoding, stale formula comments
...
**Ability**:
A character skill defined in `definitions/abilities.toml`. Fields: `name`, `description`, `type` (active/passive), `max_level` (0 = levelless ability), `tags` (array of string labels for plugin queries, e.g. `["combat_active", "sword"]`), `battle_effects` (combat effect references — see **Battle effect entry**; active: per-use / passive: always-on), optional `time_cost` (minutes, active only), optional `condition` (usage condition, active only), optional `unlocks` (array of `{at_level, ability, talent?}` for skill-tree progression). Stored on character entity as a map: `abilities = { 华山剑法 = 3 }`. Accessed in conditions via `character.{id}.abilities.{abilityId}` (returns level). Modified via `set_field` effect. Plugins query abilities by tag via `ctx.api.call('engine', 'abilities.getByTag', charId, tag)` returning all matching abilities, or `hasTag` for boolean check. Plugins declare expected tags in `plugin.toml` `required_ability_tags` / `optional_ability_tags`. Tags are free-form strings — the engine provides the mechanism (tags field + query API), plugins provide the semantics (which tags they expect), mods provide the content (which abilities get which tags). Same pattern as location tags.
_Avoid_: Ability as an attribute, ability system plugin (unnecessary for MVP), ability binding (wrong granularity — abilities are one-to-many, not one-to-one), engine-hardcoded tag names, using the generic `effects` field on an ability (that form belongs to effect-system; combat effects live in `battle_effects`)

**Battle effect entry**:
A named combat effect with defaults, shared by many skills. Defined in `definitions/battle-effects.toml` as `[effects."名字"]`; referenced from `abilities[].battle_effects` (and `talents[].battle_effects`, passive abilities) as `{ effect = "名字", 参数… }`. The reference may only override a fixed **parameter whitelist** (`chance`/`value`/`growth`/`stacks`/`turns`/`merge`/`max_stack`/`uses`/`priority`/`condition`/`when_skill`/`min_level`); structural fields (action / target / timing / category / stat / channel) are decided by the library entry alone — anything else must be written as its own library entry. **The entry is the skill's "词条" (what this move does), not the thing that ends up on a combatant** — the two are distinct objects: `delivery = "instant"` (default) executes in place at its `trigger` phase and never enters the zone; `delivery = "zone"` is a **combat state** that gets mounted onto a combatant and settles at its `settle` phase (omitted = permanent stat/channel modifier). `value` is a number (flat), `{flat, percent}` or `{set}` (channel override only); `growth` scales it per stack: `value × (1 + growth×(stack−1))`. Zone entries are mounted by a registered **applier** (`apply`, default `mount`; e.g. `apply_poison`, `apply_element`). The term is deliberately **not** "词条" — that word already means "default data entry" (`docs/native-entries-catalog.md` 原生词条 / talk-common 行为词条) in this project.
_Avoid_: Calling them 词条, calling both the entry and the mounted state "效果", inlining full effect definitions in skills (defeats the shared library), overriding structural fields from the skill line, `mode`/`k`/`merge_group` (removed in contract v4.0)

**Combat state**:
A live effect instance residing on a combatant during a battle — what a UI shows as BUFF/DEBUFF. Stored in `combatant.zone` (exposed as `getCombatState().combatants[id].effects[]` with display name, `stack`, `remainingTurns`, `category`, `trigger`, `value`, `origin`, `originId`). Its `id` is the **library entry name** (so two different skills applying 流血 share one instance and merge by `merge`), and its `origin` records who put it there: `skill` (mounted by a skill's effect entry; `originId` = skill id), `passive` / `talent` (compiled at battle start from a passive ability / combat talent — these use a composite id like `passive:内功心法#回血` and are forced to `duration = "battle"`), or `system` (mounted directly via the plugin API). A combat state is *not* a status-system status effect — status-system owns out-of-combat states in `definitions/status-effects.toml`; combat states live only inside one battle and are never written back.
_Avoid_: Confusing it with a battle effect entry (the library definition), with status-system's `status_effects`, parsing the id string to learn the origin, treating `zone` as a flat list (it holds skill-mounted debuffs, passive/talent constants, and untriggered permanent modifiers alike — filter by `origin`/`category`)

Uniqueness is **per id, not per name**: skill↔skill mounts merge (same id), but a skill-mounted instance and a passive/talent-compiled instance of the same entry coexist as two independent instances that each tick their own duration. Merging across origins was rejected deliberately — it would neuter a "temporary boost" skill (the constant instance would swallow or downgrade it), and conversely a passive constant must not be overwritten by a temporary buff. `origin` makes that split visible without parsing id strings, and it is load-bearing for three concrete things: **dispel/cleanse** must only touch `origin='skill'` entries (passive/talent entries are innate capability, not a removable affliction), the **UI** must render passive/talent entries as "常驻" rather than "0 turns" and can list only real afflictions in the buff bar, and **logs** can attribute a tick to its source (「【内功心法】回复 200 气血」).
...
**Character behavior**:
Layered position calculation for characters. Layer 1 (MVP): `home_locations` weighted (auto-normalized) + `time_rules` + `activity` probability (0=never moves, 0.5=50% chance per hour trigger). Layer 2: following relationships (implemented 2026-08-10 by follow-system). Layer 3: group/social network effects. Position is computed on demand when the player enters a location — iterate all characters, filter by home_locations matching the target area. Secondary indexes are not needed for MVP (600-char traversal is sub-millisecond). AI movement runs on every `game:hour_changed` for ALL characters (not just current+adjacent), with `activity < 0.3`降频 (every 5 hours via `hour % 5 == 0`). Movement decision: time_rules first (hour_range match → weighted random target), then home_locations re-weighted, no home_locations → no move. Characters with `sp_flag.offline` or controlled by follow-system (`is_follow ≠ 0`) are skipped by the regular AI.
_Avoid_: Full-world simulation, frame-driven AI movement, premature indexing, only simulating current+adjacent characters

**Follow**:
The companionship system (同行), owned by the follow-system plugin, replicating erArk's `sp_flag.is_follow` int semantics: 0 not following / 1 intelligent (followers at the player's location teleport-sync on player move, arriving together; regular AI skips them via `follow.isControlled`) / 2 forced (move to player position every game hour) / 3 removed (Arknights-specific) / 4 summon (TODO — stored + warning, AI not implemented, premise `IS_FOLLOW_4` registered as a reminder). Auto-end on: fatigue (optional binding `hp` ≤ 1, unbound = skipped with warning), character offline (`character:offline`), hidden-sex start (h-hidden calls the follow API). Followers do not greet on arrival (dialogue-system `registerSceneCharFilter` greet filter — generic scene-char suppression hook). Condition mirrors `character.{id}.following` (boolean) and `character.{id}.follow_mode` (number) live on the entity top level, kept in sync with `sp_flag.is_follow` by the single write point `setFollowMode`. No follower count limit (erArk's `follow_count` is dead code). Events: `follow:started` `{character, mode}`, `follow:ended` `{character, reason}`.
_Avoid_: Boolean follow state, per-system instruction folders (system-level instructions live in the native-instructions plugin), direct writes to sp_flag.is_follow from other plugins (use the follow API)

**Offline**:
A character lifecycle state (前置, 2026-08-10) where the character leaves the active world: `sp_flag.offline = true`, `current_location` cleared, `character:offline` `{id, reason?}` emitted. Owned by character-system (`setOffline`/`setOnline`/`isOffline`); the offline event is the universal cleanup contract — each plugin owning "in-world activity state" cleans its own domain (follow-system ends follow with reason `offline`; h-core/hidden/status handlers are TODO hooks until 装袋/打晕 instructions land). Identity data (attributes/relations/items/experience) is never touched. Offline characters are skipped by AI movement and are not re-placed on load. Future instructions (bagging, knocking out) call `setOffline`.
_Avoid_: Centralized cleanup in character-system (layer violation), per-instruction ad-hoc offline cleanup, re-placing offline characters on load
