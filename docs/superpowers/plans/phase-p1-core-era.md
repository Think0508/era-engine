...
### 指令栏分组（基于 erArk 分析）
- Act_COM 顶部显示类别开关行：`[★收藏夹] [日常] [猥亵] [战斗] [自定义]...`
- 每个类别是**独立开关**（非页签）：点击循环 true(亮)/false(灰)
- 所有 true 的类别指令**合并显示**，按 priority 排序
- ★ 收藏夹：已收藏的指令在原类别和 ★ 下**各出现一次**（类似软链接）
- ★ 与小键盘快捷指令联动（收藏的指令同时出现在小键盘快捷区）
- 指令定义加 `category` 字段（daily/obscenity/sex/combat/system/custom 等）

### CONF 配置变量（后续扩展）
- 用于更复杂的条件显隐（如"逆推:不会拒绝"模式）
- Ex_COM 顶部显示 `[CONF]G=[值1]=[值2]=[值3]`，点击循环切换值
- Phase P1 先不做，等 H 子系统需要时加

### TODO / Bug
- [ ] 小键盘快捷指令显示 command id（如 h_touch_hand）而非 label——bug，后续修
