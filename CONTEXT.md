...
**Battle effect entry**:
A named combat effect with defaults, shared by many skills. Defined in `definitions/battle-effects.toml` as `[effects."名字"]`; referenced from `abilities[].battle_effects` (and `talents[].battle_effects`, passive abilities) as `{ effect = "名字", 参数… }`. The reference may only override a fixed **parameter whitelist** (`chance`/`value`/`growth`/`stacks`/`turns`/`merge`/`max_stack`/`uses`/`priority`/`condition`/`when_skill`/`min_level`); structural fields (action / target / timing / category / stat / channel) are decided by the library entry alone — anything else must be written as its own library entry. `delivery = "zone"` means the effect resides on a combatant and settles at its `settle` phase (omitted = permanent stat/channel modifier); `delivery = "instant"` (default) means it executes in place at its `trigger` phase. `value` is a number (flat), `{flat, percent}` or `{set}` (channel override only); `growth` scales it per stack: `value × (1 + growth×(stack−1))`. Values are applied by a registered **applier** (`apply`, default `mount`; e.g. `apply_poison`, `apply_element`). The term is deliberately **not** "词条" — that word already means "default data entry" (`docs/native-entries-catalog.md` 原生词条 / talk-common 行为词条) in this project.
_Avoid_: Calling them 词条, inlining full effect definitions in skills (defeats the shared library), overriding structural fields from the skill line, `mode`/`k`/`merge_group` (removed in contract v4.0)
