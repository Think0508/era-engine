...
// handler ctx 约定：完整 GameContext，sourceId = 被决策的 NPC id（selectedCharacterId 同 NPC）。
//
...
function charOf(ctx: GameContext): any {
  const id = ctx?.sourceId ?? ctx?.selectedCharacterId
  return id ? entitySystem.get('character', id) as any : null
}
...
export function registerAiPremises(): void {
  ...
  conditionEngine.registerPremise('AI_TIRED', (ctx: GameContext) => {
    const char = charOf(ctx)
    return !!char?.sp_flag?.tired ? 1 : 0
  })

  // 注释：AI_NIGHT——夜晚（22:00-5:59，对齐 game:night_start=22）；权重 1
  conditionEngine.registerPremise('AI_NIGHT', (ctx: GameContext) => {
    const hour = ctx.time?.hour ?? 0
    return hour >= 22 || hour < 6 ? 1 : 0
  })

  // 注释：AI_DAY——白天（6:00-21:59）；权重 1
  conditionEngine.registerPremise('AI_DAY', (ctx: GameContext) => {
    const hour = ctx.time?.hour ?? 0
  ...
  conditionEngine.registerPremise('AI_WORK_TIME', (ctx: GameContext) => {
    const char = charOf(ctx)
  ...
  conditionEngine.registerPremise('AI_ENTERTAINMENT_TIME', (ctx: GameContext) => {
    const hour = ctx.time?.hour ?? 0
  ...
  conditionEngine.registerPremise('AI_HOME', (ctx: GameContext) => {
    const char = charOf(ctx)
  ...
  conditionEngine.registerPremise('AI_NOT_AT_HOME', (ctx: GameContext) => {
    const char = charOf(ctx)
    const home = char?.behavior?.home_locations as Record<string, number> | undefined
  ...
  conditionEngine.registerPremise('AI_IMPRISONED', (ctx: GameContext) => {
    const char = charOf(ctx)
