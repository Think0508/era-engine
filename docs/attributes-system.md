...
- **规则（写新代码必守）**：**要写回哪个值，就从哪个值出发读**；白字（基础）会被写，绿字（加成）只用于显示与判断。
  ...
  - ⚠️ **本层三处残留未决（2026-09-23 记录，均需人类裁定，本波刻意未实现）**：
    1. **`realtime-settle` 里仍有五处「判据 + 写回」把有效上限写进裸值**（settleSemen / settleTired /
        settleHunger / settleUrine / settleEjaDecay 调 `clampAttrValue(...)` 后直接 `setEntityAttr`）：
       这些站点里多数有"裸值未达上限"的前置判据（故不会回收裸值），但 `settleEjaDecay` 没有上限判据——
       临时「射精欲上限 −N」修正下会把高于新上限的裸值截断（与上面已关闭的 R1 同形）。按新模型，
       这五处应当写"无上限钳制的值"（读时封顶负责不超上限）。**需人类裁定，本波未动。**
    2. **宝珠转换的判定读有效值**（`src/core/juel-settle.ts:34-55`）：转换读**有效**状态值、清零写**基础值**，两个症状：
       (a) base 快乐 0 挂 `快乐 +500` 修正 → 每晚睡眠结算凭空产出 **500 快乐珠**（可重复，与玩家行为无关）；
       (b) 修正把有效值压到 0 时 `statusValue === 0 → continue` 分支直接跳过 → **基础值不被清零**（残留）。
    3. **跨属性家族里的反馈回路**：`src/plugins/h-core/effects/settle-effects.ts:371`（有效**阴茎大小** →
