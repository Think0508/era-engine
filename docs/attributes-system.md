...
- **规则（写新代码必守）**：**要写回哪个值，就从哪个值出发读**；白字（基础）会被写，绿字（加成）只用于显示与判断。
  ...
  - **上限/门槛判据仍用有效值**（`getEntityAttr` / `clampAttrValue`）——「上限 +500」这类修正的意义就是抬高上限，
    判超限时必须看到加成后的上限。`applyAttrDelta` 的 `clamp: true` 仍按此解析上限
    （末轮补充：它作为**写路径**另加「非负增量不得被钳制反噬」守卫，见下）
  - ⚠️ **本层三处残留未决（2026-09-23 记录，均需人类裁定，本波刻意未实现）** —— 都是「读有效值 / 写基础值」的余量：
    1. **钳制的「越顶」方向**（`src/core/entity-utils.ts` 的 `applyAttrDelta({clamp:true})`）：结果写进**基础值**，
       上限判据却读**有效**上限。「截断」方向已关闭 —— `delta ≥ 0` 时以 `old` 为下界（钳制只限制本次增量的
       幅度，绝不反向）：临时上限减益**可以**让值不再增长，但不得回收已经存在的基础值（修复前探针：
       base 体力 100 / 上限 120 挂 `体力上限 −100`（有效上限 20）→ `recover_permil +50` 把基础体力写成 **20**，
       撤掉修正仍是 20 = 永久 **−80**；`rest`/`recover_permil` 正常路径可达）。
       **「越顶」方向未关闭**：base 精液量 50 / 上限 100 挂 `精液量上限 +100` → 基础值可爬过**基础**上限到
       **200** 且撤修正后不回落 —— 与「base ≤ base 上限」不变量（`clampHpMp`、战斗 mp 回写已按此实现）冲突。
       改法（一行）：`clampAttrValue` 里 `rule.maxAttr` 的解析从 `getEntityAttr` 换成 `readRawAttr`；
       **代价**是 `src/core/entity-utils.test.ts` 的「`opts.clamp` 按修正后的上限钳制」用例
       （期望 `{old:90, new:600}` —— 600 是**有效**上限，基础上限 500）必然变红，而那条用例钉住的正是本行为，
       **动手前必须先裁定它的期望**（`clampAttrValue` 本身保持不变：它是判据，判据看有效上限是对的）。
